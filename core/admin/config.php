...
function remove_admin_menus() {
    ...
    remove_menu_page( 'edit.php?post_type=acf-field-group' ); //Custom Filed
    remove_menu_page( 'cp-post-permalinks' ); //Custom Filed
    remove_menu_page( 'duplicator-pro' ); //duplicator-pro
    remove_menu_page( 'wphpuw' ); //hide error report
    // remove_menu_page( 'wpcf7' ); //remove contact form 7
    remove_menu_page( 'itsec' ); //remove
    remove_menu_page( 'social-warfare' ); //remove 
    remove_menu_page( 'admin.php?page=woo-viet' ); //remove
    remove_menu_page( 'woo-variation-swatches-settings' ); //remove
...
function remove_admin_submenus() {
    ...
    remove_submenu_page( 'themes.php', 'customize.php?return=%2Fwp-admin%2Fnav-menus.php' ); // Appearance - Customize
...
function change_footer_version() {return 'Development by ThuongDQ 0984.13.9696';}
...
add_action( 'admin_menu', 'rename_contact_form_admin_menu', 999 );
function rename_contact_form_admin_menu()
{
    global $menu;
    // Pinpoint menu item
    $woo = recursive_array_search_php( 'Contact Forms', $menu );
    // Validate
    if( !$woo )
        return;
    $menu[$woo][0] = 'Đặt gạch - Trả góp';
}
